...
export default async function handler(request) {
  ...
  try {
    ...
    // Parse request body
    const { tableId, recordId, fields } = await request.json()

    if (!tableId || !recordId || !fields) {
      return createErrorResponse('tableId, recordId, and fields are required', 400)
    }
    ...
    return createSuccessResponse(data, 'Record updated successfully')
