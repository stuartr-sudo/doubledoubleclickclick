...
export default function UserDashboard({ user }) {
  ...
  // robust dedupe helper (keeps newest)
  const dedupeItems = (items, kind) => {
    ...
    for (const it of sorted) {
      ...
      const userName = it.user_name || '';
      const key =
        (it.processing_id && `${kind}:proc:${it.processing_id}`) ||
        (it.id && `${kind}:id:${it.id}`) ||
        `${kind}:user:${userName}:title:${normTitle}`;
  ...
  return (
    <div className="p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {user.assigned_usernames?.length === 0 && (
            ...
            <div className="space-y-4">
              ...
              {recentPosts.length === 0 && (
                <p className="text-slate-500 text-center py-8">
                  No posts found for your assigned usernames.
                  {user.assigned_usernames?.length === 0 && (
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              {recentWebhooks.length === 0 && (
                <p className="text-slate-500 text-center py-8">
                  No new content assigned to you.
                  {user.assigned_usernames?.length === 0 && (
                    <span><br />Contact your admin to assign you usernames.</span>
                  )}
                </p>
