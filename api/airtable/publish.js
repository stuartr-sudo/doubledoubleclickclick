...
export default async function handler(request) {
  ...
  try {
    ...
    // Parse request body
    const { tableId, recordId, title, content } = await request.json()

    if (!tableId || !recordId || !title || !content) {
      return createErrorResponse('tableId, recordId, title, and content are required', 400)
    }
    ...
    // Update record in Airtable with title and content
    ...
    return createSuccessResponse({
      success: true,
      record: data
    }, 'Content published to Airtable successfully')
